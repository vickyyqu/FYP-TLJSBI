...
def build_forecast_model(category):
    ...
    future = best_m.make_future_dataframe(freq='W', periods=52, include_history=False)
    future['rank'] = forecast_rank
...
def forecast_popular_categories():
    ...
    sorted_df = pd.DataFrame(data=sorted_categories)
    sorted_data = sorted_df.to_csv(index=False)
    key = f"starbuy_categories/{label}_results.csv"
    s3.put_object(Bucket=s3_bucket_name, Key=key, Body=sorted_data)
